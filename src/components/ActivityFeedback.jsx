...
const TIER_CONFIG = {
  hint:    { icon: '💡', color: 'var(--amber)',    bg: 'var(--amber-glow)',   border: 'rgba(245,158,11,0.25)' },
  lesson:  { icon: '📖', color: 'var(--primary)', bg: 'var(--primary-glow)', border: 'rgba(99,102,241,0.25)' },
  praise:  { icon: '⭐', color: 'var(--success)',  bg: 'var(--success-glow)', border: 'rgba(16,185,129,0.25)' },
};
...
export default function ActivityFeedback({ result, activityType, onDismiss }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'flex-end', justifyContent: 'center', zIndex: 200, padding: '1rem' }}
      onClick={onDismiss}>
      <div style={{ background: 'var(--surface)', borderRadius: 'var(--radius-lg) var(--radius-lg) 0 0', padding: '1.5rem', width: '100%', maxWidth: 620, maxHeight: '80vh', overflowY: 'auto', boxShadow: '0 -8px 32px rgba(0,0,0,0.5)' }}
        ...
        {result.suggestion && (
          <div style={{ background: 'var(--surface-2)', borderRadius: 'var(--radius-md)', padding: '0.875rem', marginBottom: '1.25rem' }}>
            <div style={{ fontSize: '0.7rem', fontWeight: 700, textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: '0.375rem' }}>💬 Suggestion</div>
        ...
        {questionResults.length > 0 && (
          <div>
            <h4 style={{ fontSize: '0.85rem', fontWeight: 700, marginBottom: '0.75rem', color: 'var(--text-muted)' }}>Question Results</h4>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.625rem' }}>
              {questionResults.map((qr, i) => (
                <div key={i} style={{ display: 'flex', gap: '0.625rem', padding: '0.625rem', background: 'var(--surface-2)', borderRadius: 'var(--radius-sm)' }}>
                  <span style={{ flexShrink: 0, fontSize: '1rem' }}>{qr.correct ? '✅' : '❌'}</span>
                  <div style={{ flex: 1, fontSize: '0.8rem' }}>
                    {qr.prompt && <div style={{ color: 'var(--text-muted)', marginBottom: '0.25rem' }}>{qr.prompt}</div>}
                    <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
                      {qr.user_answer != null && qr.user_answer !== '' && (
                        <span>Your: <strong>{String(qr.user_answer)}</strong></span>
                      )}
                      {!qr.correct && qr.correct_answer != null && (
                        <span style={{ color: 'var(--success)' }}>Expected: <strong>{String(qr.correct_answer)}</strong></span>
                      )}
                    </div>
                    {qr.feedback && <div style={{ marginTop: '0.25rem', color: 'var(--text-muted)', fontStyle: 'italic', fontSize: '0.78rem' }}>{qr.feedback}</div>}
                  </div>
                  {typeof qr.score !== 'undefined' && (
                    <span style={{ fontSize: '0.8rem', fontWeight: 700, color: 'var(--primary)', flexShrink: 0 }}>{qr.score}pts</span>
                  )}
                </div>
              ))}
            </div>
