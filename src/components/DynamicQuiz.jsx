        ...
        return (
          <button key={i} type="button"
            ...
            style={{
              ...
              border: `2px solid ${isCorrect ? 'var(--success)' : isSelected ? 'var(--primary)' : 'var(--border)'}`,
              background: isCorrect ? 'rgba(16,185,129,0.1)' : isSelected ? 'var(--primary-glow)' : 'var(--surface-2)',
              cursor: showResult ? 'default' : 'pointer', fontSize: '0.875rem', transition: 'all 0.15s',
            }}>
            <span style={{ marginRight: '0.5rem', fontWeight: 700, color: 'var(--text-muted)' }}>{label}.</span>
            {optText}
          </button>
        );
      })}
...
function FillBlankRenderer({ qid, answer, onChange, blankedSentence }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
      {blankedSentence && <p style={{ fontSize: '0.9rem', color: 'var(--text-muted)' }}>{blankedSentence}</p>}
...
function MatchingRenderer({ qid, pairs, answer = {}, onChange }) {
  if (!pairs || pairs.length === 0) return <p style={{ color: 'var(--text-muted)', fontSize: '0.85rem' }}>No matching pairs defined.</p>;
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
      {pairs.map((pair, i) => (
        <div key={i} style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
          <div style={{ flex: 1, padding: '0.5rem 0.75rem', background: 'var(--surface-2)', borderRadius: 'var(--radius-sm)', fontSize: '0.875rem' }}>{pair.left}</div>
